...
- **But "cloud is cheaper than satellite" is not established.** For an always-on, redundant feed with 24/7 support, the fully-loaded cost of an Internet-native path — egress, relay compute, redundancy, NOC — does not obviously beat a *depreciated* transponder or an existing IP contract at scale, and egress pricing can dominate the model. The case is *route-specific and still to be proven with numbers*: strongest where the incumbent case is weakest (dynamic, short-lived or long-tail topologies), and increasingly achievable for trunk routes as incumbent capacity retires and egress pricing falls.
...
4. **Redundancy is engineered into the fabric, not bolted on** — dual-path and hitless switching, regional relay redundancy and graceful degradation expressed through policy rather than per-route engineering. This is the least complete goal: endpoint reconnect and fan-out work, and active/active *source* failover across a relay mesh passes end to end only on an open upstream fix, where recovery is bounded by the QUIC idle timeout rather than hitless. Redundancy today therefore means doubling the whole chain and letting the receiver select ([evidence](evidence.md) §7).
...
- **The economics work on some real, identifiable class of routes:** most likely dynamic, short-lived or long-tail routes first, expanding toward trunk routes as the incumbent cost base erodes, rather than a wholesale day-one replacement.
