...
- **Congestion behaviour degrades rather than blocks.** QUIC's per-stream delivery avoids serialising a whole multiplex behind one lost packet; whether that beats a well-tuned SRT/RIST link is unmeasured, and the incumbent UDP transports do not share a single blocking flaw ([transport](transport.md) §3.1, §9). The advantage is strongest on the media-aware lane and constrained on the opaque fallback ([architecture](architecture.md) §14.2).
