...
Layer: **the requirement** — what primary distribution has to do, before any data plane is chosen.
...
## 6. Why MoQ fits, and what else does

**MoQ enters only now, as a candidate that happens to match requirements the market is generating on its own — not as a cause of the change, and not as the only candidate.** Segmented HTTP carrying MPEG-TS is the other, and the two are compared axis by axis in [alternatives](alternatives.md); this section states MoQ's fit, and the paragraphs that close it state where that fit is weaker than the alternative's. It is a live-media transport being standardised in the IETF and implemented in the open, and what makes it *architecturally* interesting, as distinct from merely performant, is that several native properties line up with the §4 pressures:

- **Native relay and 1:N amplification (the advantage usually cited first).** One protocol carries a feed from contribution through a relay fabric that fans out point-to-multipoint, with subscription-based cluster routing and caching in the model rather than bolted on. This maps directly onto primary distribution's "trunk that must occasionally fan out globally and dynamically" shape (§4.4), and it is what an SRT/RTP point-to-point tunnel does not give you natively. Its *cost* value is narrower than its architectural value, though: the relay removes duplicated backhaul and uplink, while last-mile delivery stays linear in destinations for MoQ exactly as for everything else ([economics](economics.md) §4.8).
- **It is shaped like something a commodity delivery market can sell — the strongest economic argument in this paper, a claim about markets rather than a measurement, and one that applies to the sub-second band rather than to delivery in general.** A relay is a cache: it receives an object once and serves N subscribers over N unicast connections, which is what a CDN edge already does and already prices. SRT has no object model and no native relay, so fanning it out means running a stateful media gateway per stream per destination — a media-server business rather than a delivery business, which is why no CDN sells SRT relay as a commodity product and why one already sells MoQ relay. SRT *can* still be scaled, through a re-origination tier the operator runs itself; what is unavailable is a market that sells it, so the burden and the cost basis stay with the operator ([economics](economics.md) §4.9). Openness is necessary but not sufficient here, since SRT is open too and produced no commodity relay market. If CDNs deploy MoQ relays and compete, relay capacity follows commodity delivery pricing rather than hyperscaler primary-distribution pricing, which moves the viable destination count by more than any engineering change available. Two limits travel with the argument. It needs relay portability between implementations, which is currently absent in practice ([evidence](evidence.md) §9), because a market cannot commoditise a product buyers cannot switch between. And the market position it aims at is one **segmented HTTP already holds** — so the claim is not that MoQ is the cheapest way to move a feed over the internet, only that it is the first candidate that could bring commodity pricing to the sub-second band ([alternatives](alternatives.md) §10).
...
- **The path back to a transport stream is one implementation.** For segmented HTTP, reassembly is off the shelf in tooling broadcast engineers already run — `tsp -I hls` and ffmpeg both read a playlist — and professional IRDs and edge gateways list HLS and DASH inputs beside RF, ASI, SRT, RIST and Zixi. For MoQ, `moq export ts` is the only stage that converts tracks back to MPEG-TS. What this does *not* buy is the grooming half: because a distributor no longer supplies its clients' receivers, a clean paced TS at the demarcation is the distributor's obligation on both data planes, and no off-the-shelf tool produces one ([alternatives](alternatives.md) §4).

MoQ is not the first protocol to point at an Internet-native future — low-latency DASH, WebRTC/SFU fan-out, TS-over-HTTP and its own WARP lineage share that ancestry. **The strongest of them is segmented HTTP carrying MPEG-TS, and it is not a historical predecessor but a live alternative that is currently ahead of MoQ on most of the axes an operator decides on** — interoperability, delivery economics, operational maturity, the robustness of its recovery model, and the availability of an off-the-shelf path back to a transport stream ([alternatives](alternatives.md)). What it is *not* ahead on is the broadcast-grade edge layer itself, which is the distributor's to build on either data plane. What distinguishes MoQ is combining native relay/amplification, subscription-based delivery and token-scoped authorization in one CDN-deployable protocol at sub-second latency that segmented HTTP structurally cannot reach. That is a real difference and a narrow one, and it makes latency the axis on which the choice of data plane is actually decided.

The consequence is a wider frame, not a different conclusion. **There are two viable Internet-native data planes, not one, and everything that makes either of them broadcast-grade sits above the transport and is common to both**: PCR and CBR grooming to TR 101 290, 1+1 with byte-identical legs and receiver-side selection, ST 2022-7 pairing, entitlement and multi-tenant control, observability in broadcast terms, and interop with the MPEG-TS installed base. Neither specification addresses any of it. §1 said the transport would be the least interesting part of the transition; this is what that looks like when it turns out to be true, and it is why the transport is treated here as a necessary substrate rather than as the value.

---

## 7. The long-term vision: a distribution fabric

**The end-state is not "satellite, but over the Internet" but a qualitatively different operating model: a *distribution fabric*, a software-defined, globally reachable, multi-tenant layer over which linear feeds are provisioned, entitled, delivered and observed as software services** (assuming §5 holds and one of the two data planes in §6 provides the substrate). Its defining properties, as design goals rather than achievements:
...
6. **The transport is interchangeable** — and this is now a measured claim rather than a design aspiration. Because the durable value sits in the control, entitlement, egress and observability layers, the fabric treats the data plane as a substitutable component: MoQ where sub-second matters, segmented HTTP where commodity delivery and universal interop matter more, and established transports where neither is ready. §6 sets out how that choice is made; the point here is that making it differently does not change the rest of the fabric.
...
- **The value layer is genuinely defensible.** Since the transport commoditises, durable value must accrue in the control-plane, entitlement, egress-interop and observability layers. That is not guaranteed, and the challenge is a crowded control-plane market: MediaConnect, Zixi, LTN and others ship capable management planes. The egress-interop layer is on firmer ground than it first appeared — because the distributor cannot push a clean-TS hand-off onto its clients' receivers, that layer is required and owned on both data planes ([alternatives](alternatives.md) §4). What remains untested is whether a bought ABR-to-TS gateway grooms to broadcast conformance; if it does, part of the layer is purchasable on the segmented-HTTP side, and defensibility narrows to doing the job materially better rather than to occupying the layer.
- **The sub-second requirement exists on identifiable routes.** This is the condition on which MoQ specifically, rather than Internet-native distribution generally, depends. Everything else on which the two data planes differ currently favours segmented HTTP, so if no real route needs sub-second delivery then MoQ addresses a preference rather than a requirement ([alternatives](alternatives.md) §5).
