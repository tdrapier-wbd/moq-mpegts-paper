...
requirement set; this document evaluates the candidates against R1–R7, and against the point-to-point
incumbents, on the axes an operator actually decides on. R6 and R7 appear here only as far as the
*data plane* decides them — the protocol's redundancy and enforcement behaviour — while the
broadcast-grade engineering above them, which is common to both candidates, is
[Architecture](architecture.md) §5 and [Control plane](control-plane.md). R8 is not a data-plane
axis and is not scored here. The layer above the transport — where
...
| Marginal cost of a destination | a cache fill: shared upstream, one egress copy | **0.806 % of a relay core, 1.39 MB of relay memory and one full stream copy** — linear, with no superlinear term and no per-subscriber cache duplication ([Evidence](evidence.md) §3.6) | a session's worth of gateway CPU and one egress copy |
...
| Equal sizes, re-measured: byte-faithful receiver, controller pinned | 0.485 | **0.254–0.263** | **0.000** BBRv3 / **0.039** CUBIC |
...
flag, where the segmented lane on HTTP/3 keeps a quarter of the stream whatever the controller.
The segmented range spans two origin configurations — 0.254 at nginx's default HTTP/3 stream buffer,
0.259–0.263 with it raised — with 11 holes a cell and so void for carriage, and neither is
controller-dependent; a lower reading taken at the default buffer was a capture the receiver's
per-fetch timeout ended. On TCP the
...
restarts it stalled video and primary audio permanently, bisected to its own merge commit
...
| Null stuffing | **carried** — measured | not the source's; since the upstream mux-rate change (below) the exporter pads to the declared rate (4.93 %, file domain) | carried if verbatim | **carried** — measured |
| Mux rate | preserved | declared since that change, **+0.36 %** on the source (file domain); before it, none | preserved if verbatim | **the source value exactly** — measured |
...
| **Publish / package** | `moq import ts` (media-aware) or an opaque lane under MSFTS | *classic:* `tsp -O hls`<br>*low-latency TS:* Apple `mediastreamsegmenter --format=transport` | distributor |
