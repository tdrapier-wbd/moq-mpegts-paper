...
| P1-m | **The SRT arm on the T28/T31 ladders, matched on *measured* latency** — does MoQ match SRT's residual loss for the same milliseconds of delivered buffer? | [T28](test-28-failure-injection-matrix.md) § *The budget ladder replicated* | — **partly run, and one of its two rig blockers is now cleared.** The MoQ half is done (3 reps × 6 budgets, clean controls) and discharged T28's non-monotonicity replication. **The mirroring tap is built and validated**: the artefact was the *source-side* pass-through tap alone — it reproduces 4.563–4.693 s lost and 6,001–6,493 continuity errors, while the egress tap inline grades 0.000 s like no tap at all — and `tsp -P fork --nowait` removes it at an unchanged picture count ([T28](test-28-failure-injection-matrix.md) § *The artefact attributed*). **Remaining: match SRT's `--latency` to the MoQ lane's measured delivery latency rather than its nominal budget, then re-run the ladder.** Nothing upstream blocks it. |
