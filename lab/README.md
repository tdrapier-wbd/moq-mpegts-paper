...
| T19 | Verifying the upstream PCR grid fix, and where it stops | supports Gate 2 (conformance) | grades [#2967](https://github.com/moq-dev/moq/pull/2967) and [#3006](https://github.com/moq-dev/moq/pull/3006), the fixes for the defect T18 measurement 6 diagnosed and this campaign filed | complete, and it **splits into a clean pass and a clean fail on the same build**, across three domains. **Values:** fixed exactly — every one of 2,472 intervals at **25.000 ms**, above 40 ms 210 → **0**, sub-millisecond clustering 85.40 % → **0.00 %** — plus a conformance defect we never found (reserved PCR bits `0x00` → `0x3F`) and the 11 µs mechanism explained from the code. **Arrival time:** #3006 paces the stdout writer, and it works — on-grid share 27.4 → **56.9 %**, gate failures 18.26 → **7.45 %**, median 24.69 ms — but bimodally, with 28.9 % arriving in bursts. **Byte positions:** untouched and untouchable by a timing fix, so `tsp -P pcradjust` still regenerates the original distribution from them (293 above 40 ms, 87.9 % sub-millisecond) and our byte-locking groomer still drops **45.9 %** of content. End to end the lane is unchanged by #3006 — 120 → **772 ms**, continuity 0 → **1,166** — so the deployable build is still the pre-fix one. The remaining ask was the *positional* one, filed as [#3334](https://github.com/moq-dev/moq/issues/3334) with the three-domain instrument offered as [#3335](https://github.com/moq-dev/moq/pull/3335) — and it was **granted, merged and graded on the wire** as [#3351](https://github.com/moq-dev/moq/pull/3351): at the pipe, adjacency **87.2 % → 0.0 %** and releases outside ±10 ms **2/4,779** at a p95 of **1.70 ms**, with upstream's own gate green on the merged build. **On the wire the gate still fails.** Against #3351's own merge-base the byte-locking groomer drops **36 % fewer** packets and halves its stuffing — the first movement this lane has had on content — while delivery latency goes **776.8 → 2,126.2 ms**, continuity is 811 errors and repetition above 40 ms goes 10.8 % → **12.2 %**. **The residue is not a defect:** a coded frame's bytes belong to its own 40 ms, so a 417 kB I-frame is 357 ms of carrier, and the CBR mux schedule that used to smooth that is not in the decode timestamps. It has to be bought downstream, and it can be — cushioned past the bounded **761 ms** displacement, the programme is **99.6 %** conserved at 0 continuity errors and exact CBR. The **~480 ms standing lag is neither B-frame reorder depth nor the latency budget**: `bframes=0` still carries 428.6 ms of it, and moving `--latency-max` across 500 ms / 1 s / 2 s moves it by under 40 ms. **The instrument now has a test of its own**, which it did not when any of this was measured: ten boundary fixtures and 38 assertions (`ts-pcr-fixtures.py`, `ts-pcr-selftest.py`), with the 33-bit wrap **placed 400 ms in** rather than soaked 26.51 h for, and two further analyser defects found in the building of it — both of it failing conforming input. **Half the downstream damage turned out to be ours:** the groomer read PCR value cadence and byte-position cadence as interchangeable and exited *zero* having shed 67.2 % of a fixture that passes every value-domain check — now measured and guarded | [test-19-pcr-grid-verification.md](test-19-pcr-grid-verification.md) |
...
- **The groomed MoQ egress is not P1-conformant on PCR repetition on the wire, at any buffer depth.**
  ...
  *other* data plane at an 8 s cushion. The obvious reading — that MoQ needs comparable depth, spending
  ...
  groomer's own PCR insertions vary 137 → 0 across that ladder without moving the figure, because it can
  only place a PCR in a slot it was going to stuff and those do not fall in the exporter's gaps.
  Measured against the same clip on three transparent lanes, the exporter conserved the PCR *count* —
  ...
  **evenly spaced** exporter cadence clears the gate is still the campaign's highest-leverage outstanding
  run, and it is now half-answered: [#2967](https://github.com/moq-dev/moq/pull/2967) made the exporter's
  PCR *values* an exact 25 ms grid, verified in [T19](test-19-pcr-grid-verification.md), but the spacing
  is carried as per-frame timestamps that the exporter's byte-stream output discards, so the PCR *packets*
  still leave bunched. **The remaining ask is the output path**, not the placement rule
  ([planned-experiments](planned-experiments.md) item 1).
