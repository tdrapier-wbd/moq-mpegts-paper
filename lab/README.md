...
| T23 | Which PCR timeline events the lane survives, by class | Gate 2 (conformance) across the events a permanent feed cannot avoid | grades the discontinuity as a controlled variable instead of taking whatever the looping source happened to supply; supersedes T21's class-level reading | **complete, and it splits three ways.** The **33-bit PCR base rollover — the one unavoidable event, every 26.51 h — is carried correctly end to end**: a 30.080 ms step in modulo arithmetic at all three points, 6,259 PCRs within ±500 ns, 0 continuity errors, 52 ms content gap against the control's 26 ms. **Forward jumps recover** in 238 ms. **A rewind costs its own duration in programme**, one-for-one from 1 s (268 ms) to 600 s (≥62.8 s), because the exporter's scheduler is monotonic in media time; recovery is a single 18.3 MB burst which overran the groomer in the encoder-restart arm (54,168 drops, 103 continuity errors — the only ones in the campaign). `discontinuity_indicator` is neither consumed nor produced: 4 arms present it, the exported wire carries 0, and it is hardcoded `false` upstream. Measurements contributed to [#2833](https://github.com/moq-dev/moq/issues/2833). In every failing arm the wire stayed conformant across the hole |
