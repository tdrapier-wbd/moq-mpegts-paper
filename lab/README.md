...
| T12 | End-to-end 1+1 dual-path delivery and hand-off | 6 (redundancy drill) | Gate 3 — resilience; de-risks Gate 2 | complete for a co-started pair, arms A–D, incl. leg failure and recovery; independent restart of one leg blocked upstream. The service layer, clock included, is byte-identical across the pair even with one leg running behind — the exporter renders SI from media position, not arrival | [test-12-dual-path-handoff.md](test-12-dual-path-handoff.md) |
...
| T19 | Verifying the upstream PCR grid fix, and where it stops | supports Gate 2 (conformance) | grades [#2967](https://github.com/moq-dev/moq/pull/2967), the fix for the defect T18 measurement 6 diagnosed and this campaign filed | complete, and it **splits into a clean pass and a clean fail on the same build**. The exporter is fixed exactly — every one of 2,472 intervals at **25.000 ms**, intervals above 40 ms 210 → **0**, the sub-millisecond clustering 85.40 % → **0.00 %** — plus a conformance defect we never found (reserved PCR bits `0x00` → `0x3F`) and the 11 µs mechanism explained from the code. But the spacing lives in per-frame timestamps that `moq export ts`'s stdout discards, so **87.2 % of PCR packets leave back-to-back** and the clustering merely changes domain: `tsp -P pcradjust` regenerates the original distribution from the byte positions (293 above 40 ms, 87.9 % sub-millisecond) and our byte-locking groomer drops **45.9 %** of content. End to end the lane regresses — continuity 0 → 824, latency 118 → 769 ms — so the deployable build is still the pre-fix one | [test-19-pcr-grid-verification.md](test-19-pcr-grid-verification.md) |
