...
- **The groomed MoQ egress is not P1-conformant on PCR repetition on the wire, at any buffer depth.**
  ...
  *other* data plane at an 8 s cushion. The obvious reading — that MoQ needs comparable depth, spending
  ...
  Measured against the same clip on three transparent lanes, the exporter conserves the PCR *count* —
  31–36 a second against the source's 41, where ~25 would satisfy the gate — and destroys the *spacing*,
  putting 85 % of intervals under 1 ms and collecting the residue into gaps of 100 ms to 1.8 s. Whether an
  **evenly spaced** exporter cadence clears the gate is the campaign's highest-leverage outstanding run;
  it is blocked on upstream, where [#2937](https://github.com/moq-dev/moq/issues/2937) currently asks for
  a *denser* cadence and needs re-framing to placement before anyone acts on it
