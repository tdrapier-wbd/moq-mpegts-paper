...
| T21 | The permanence soak of the complete media-aware lane, groomer included | Gate 2 (conformance) over time, and the permanence question the use case rests on | the first long run to put the groomer inside the measurement; supersedes the assumption that T19's 300 s result extends | **running, and already failed its own criterion.** The wire is clean and stays clean — **0** continuity errors, **0** PCR intervals above 40 ms (worst 30.08 ms), exact 11,000,000 b/s, 0 drops, 0 respawns, programme conserved at 9.55 Mb/s. Behind it the groomer's release loop comes apart at about **nine minutes**: the recovered media rate leaves the truth and ramps **linearly without bound** — 9.34 Mb/s at t=541 s, 34.7 at t=601, **2,576.8 at t=1,202**, ~+250 Mb/s per minute — and the de-jitter buffer collapses from a standing **10,587 packets (~1.4 s) to 0** with underruns at **~970/s**. **Nothing downstream can see it**: programme is conserved so no content check fires, the carrier holds rate so no bitrate check fires, and PCR and continuity are untouched. What is lost is the cushion, and with it the lane's only protection against arrival jitter. **This is ours, not upstream's** — the exporter log is clean for the whole run. **Mechanism not located and three candidates eliminated**: the source loop wrap (does not reproduce with MoQ removed, 150 s over several wraps at a steady 9.48–9.50 Mb/s), an upstream event, and degenerate PCR intervals alone. The consequence for the paper is a **scope, not a retraction**: T19's conformance result stands for its 300 s window, the divergence begins at ~540 s, so the lane is conformant **over minutes and not established over hours** | [test-21-permanence-soak.md](test-21-permanence-soak.md) |
