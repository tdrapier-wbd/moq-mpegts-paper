...
| T21 | The permanence soak of the complete media-aware lane, groomer included | Gate 2 (conformance) over time, and the permanence question the use case rests on | the first long run to put the groomer inside the measurement; supersedes the assumption that T19's 300 s result extends | **Mechanism found; two faults in series, one upstream and one ours.** The wire is clean and stays clean throughout — **0** continuity errors, **0** PCR intervals above 40 ms (worst 30.08 ms), exact 11,000,000 b/s, 0 drops, 0 respawns, programme conserved. Behind it the groomer's recovered media rate leaves the truth at about **nine minutes** and ramps **linearly without bound** to 6.98 Gb/s, while the de-jitter buffer collapses from **10,587 packets (~1.4 s) to 0** with underruns at **~970/s**, and **nothing downstream can see any of it**. The **trigger is upstream**: the source loops the clip at 600 s and signals one clean PCR discontinuity, and the exporter never acts on it. Under *this* stimulus its PCR degenerated into a counter — see [T23](test-23-pcr-discontinuity-classes.md), which characterises the class and does not reproduce that. It latches the last pre-wrap value and emits PCR advancing **one 90 kHz tick per packet** thereafter, so 100,000 packets of programme carry **6.9 ms** of PCR instead of 15,880 ms. The **amplifier was ours**: the estimator divided real packets by a media time that had stopped advancing. Diagnosed by reporting the two accumulators separately (denominator flat at 2.15 s, numerator ramping at exactly the arrival rate) and attributed by capturing both sides of the round trip in one run. Groomer half **fixed and regression-tested** (`mpegts-pacer` `5ab84cd`); exporter half reported upstream. *The earlier reading — wholly ours, loop wrap eliminated — was wrong on both counts: the control that eliminated the wrap had removed the exporter from the path.* **The 24 h soak on a continuous timeline is now complete and the verdict splits.** Because no live source exists here and every clip is minutes long, `ts-continuous-source.py` replays one with PCR, PTS, DTS and the continuity counters advanced across the join — graded before use at 0 backward steps, 0 discontinuity flags and 0 continuity errors. **The media plane passes without qualification:** 24.01 h, **632,199,204** packets delivered and counted, **5,947,298** PCRs, **0** continuity errors in all 1,437 samples, **0** intervals above 40 ms, **0** absolute failures at ±500 ns, **0** drops, **0 underruns**, **0** respawns, exact CBR, and a worst programme gap of **27 ms across the whole day** — the same figure T23's control returns over 105 s. The **33-bit rollover was crossed in flight at 19.4 h** for nothing. The buffer-walk concern is **closed**: high water **9,903 packets set in the opening minutes and never beaten in the following 23 h**. So T19's 300 s conformance result now extends to a day. **The resource criterion fails, in one role.** The groomer is **flat** (+0.5 MB over 24 h), the exporter converged and turned over, and the relay is **logarithmic** (quarterly slopes 9.85 / 4.56 / 2.42 / 1.75 MB/h, log R²=0.9895 against 0.9097 linear, log fit extrapolating to ~519 MB at a year) — which **confirms the shape of T8b C6's asymptotic reading**, previously only a direction, while **raising its constant**: this run passes C6's baseline + 200.5 MB asymptote and reaches baseline + 242.6 MB at 24 h, so the budget is ~2.5× the slot arithmetic rather than 2.03×. But **`moq import ts` grows linearly at +2.83 MB/h** with its slope intact in every quarter (2.36 / 2.87 / 2.81 / 2.57), a linear fit at R²=0.9898 beating a logarithm (0.8960) and a square root (0.9655), and a largest drawdown from peak of 9.2 MB against 137 MB of growth — ~24 GB in a year, exhausting a 15.3 GB host in ~7.5 months. **Permanence is blocked by one upstream component, not by the architecture** | [test-21-permanence-soak.md](test-21-permanence-soak.md) |
| T22 | Silent media-plane failure: the feed stops, the transport does not | R8 (observability), the failure primary distribution is least protected against | closes P0-4 for the MoQ lane and validates [Architecture](../docs/architecture.md) §9.1's monitoring design | complete, six arms including a control. **The transport never detects a stalled source**: with the source frozen for **120 s** and every session established, publisher, relay and exporter logged **nothing** — no error, no timeout, no reconnect. Not a threshold artefact, since the 30 s and 120 s arms agree. **The media plane detects it in about one cushion**: the groomer's content alarm at **1.69–1.88 s** and PCR progression within the 100 ms observation tick, from two independent mechanisms, with **no false positive on the control**. A frozen **relay** is the one case the transport catches — QUIC's idle timeout at **34.3 s**, 18× slower, taking the egress chain with it. **`--on-stall continue` makes the failure undetectable downstream**: byte-perfect CBR, valid PCR, no programme, indefinitely. **Recovery is clean and scored on media**: back within **0.02–1.02 s** of resume, stable within 0.92 s, and the programme clock skips **exactly** the wall-clock outage (119,365 ms over 119.3 s) — the lane resumes at the live edge, so it neither replays nor runs late, and the programme lost is gone. The buffer buys **1.8–1.9 s** and no more. **Its conclusion that "PCR progression is the detector to build on" is now corrected by [T24](test-24-partial-media-plane-stall.md)**: that holds for a total stall and fails for a partial one, because PCR shares a PID with the video whose absence it is being asked to report. The property T22 valued in it — needing nothing from MoQ, nothing from the groomer and no cooperation from the sender — belongs instead to per-PID access-unit liveness | [test-22-silent-media-plane-failure.md](test-22-silent-media-plane-failure.md) |
| T23 | Which PCR timeline events the lane survives, by class | Gate 2 (conformance) across the events a permanent feed cannot avoid | grades the discontinuity as a controlled variable instead of taking whatever the looping source happened to supply; supersedes T21's class-level reading | **complete, and re-graded against the fix it prompted.** The **33-bit PCR base rollover — the one unavoidable event, every 26.51 h — is carried correctly end to end** and always was. Everything else changed: [#3375](https://github.com/moq-dev/moq/pull/3375) opened citing these measurements, merged as `0e61e3520` and closed #2833, and re-running all six arms unchanged against `d88c2ee99` puts **every arm at the control's content gap** — the 600 s rewind at 27 ms where it was ≥62.8 s, the encoder restart at 37 ms with 0 drops and 0 continuity errors where it was 44,049 ms with 54,168 and 103. The `discontinuity_indicator` is now emitted on the three signalled arms and correctly withheld on the rollover. **Residue:** a forward jump still reaches the wire unflagged, tracked upstream as `quest/m0/ts-forward-discontinuity.md` and pointed at these stimuli. Before the fix a rewind cost its own duration in programme, one-for-one from 1 s to 600 s, with the wire showing nothing — which is the durable finding and the reason the content-gap counter exists. |
| T24 | A partial media-plane stall: half the programme stops and every check stays green | R8 (observability), and the one failure mode that could undermine the observability result the architecture's case rests on | closes the partial-stall half of F3, which T22 named as its own limit; **corrects T22's primary recommendation**; supplies F10's first reportable output | **complete, four arms including a control.** `ts-partial-stall.py` suppresses chosen elementary streams for 60 s of the stream's own PCR time, substituting nulls so the mux rate is unchanged to the byte and keeping the continuity counters legal across the seam — without which the failure would be trivially detectable and the result an artefact of the tool. **The stimulus grades as healthy before MoQ sees it**: identical packet count (3,967,645), identical PCR count (24,574), identical worst interval (24.951 ms), 0 continuity errors by our grader *and* TSDuck, `pcrverify --absolute` passing — with a **60.02 s hole in the video**. **The lane contains the failure, and that is the load-bearing result**: with the video dead for a minute, audio, subtitles, SCTE-35 and the PSI tables all ran with no interruption over 2 s and the carrier held **11,001,354 b/s** against the control's 11,001,940. The objection that reassembling a mux from independently delivered tracks lets one dead track block the others is measured and **does not occur**. **But a partial stall is invisible to the whole of TR 101 290 P1**: across a **57.22 s** video hole, **0** continuity errors, **0** intervals above 40 ms and a worst interval of **30.080 ms — identical to the control**; the transport's log lines are *identical* to a healthy run's once arm names are normalised away. **What does fire is the stuffing ratio** — 13.7 % → **95.2 %**, within one second — and the **groomer's underruns**, 406,850 against 0. **Both are blind to a small stream**: killing both audio streams and the subtitles, ~440 kb/s of a 9.5 Mb/s programme, peaks stuffing at **27.0 %** against a control that peaks at **27.1 %**, and leaves underruns at 0. Sensitivity is proportional to the dead stream's share of the mux, so an alarm sensitive enough for audio would false-positive on healthy VBR video. **Only per-PID access-unit liveness caught every arm** — 57.22 s on the video, 60.53 s on the AC-3 — with nothing on the control. Recovery was clean everywhere (0 continuity errors, 0 drops); the total-loss arm took one PCR rebase at the seam and nothing else. **Upstream asymmetry:** `moq import ts` warns when the *audio* parser loses frame sync (from [#3372](https://github.com/moq-dev/moq/pull/3372)) but says **nothing** when a video track goes quiet for 57 s, though it already parses every stream to demux it. **Not tested, deliberately:** a frozen *picture* in valid advancing access units would defeat every detector here including per-stream liveness, and does so over SDI and opaque carriage equally — a limit of transport monitoring, not of this architecture | [test-24-partial-media-plane-stall.md](test-24-partial-media-plane-stall.md) |
| T25 | Isolation under abuse: can one receiver degrade the others? | R2/R7 — the multi-tenancy exposure [Comparison](../docs/comparison.md) §2 asserts and had never tested | tests a claim the paper was making unsupported; attributes a relay memory cost to a mechanism with a knob on it | **complete, five arms plus a control.** The **media plane is isolated**: every arm leaves the victims within 8 KB of the control across 198 MB at **0 continuity errors**, and the relay never refuses or delays a connection. What abuse costs is the relay's *memory* — `storm` drives RSS from **87 MB to 1.9 GB in 60 s** at 40 churning receivers — and four further arms say what that is: **abandoned sessions retained until the QUIC idle timeout**, each still accruing media it will never deliver. It is **not cumulative** (four identical cycles reach 1,911 → 1,949 → 1,955 → 1,959 MB — the first sets the high-water and the rest reuse it), **not the group cache** (capping it at 256 MiB leaves the peak unchanged, so the documented memory bound does not cover this at all), and **not concurrency** (the same 42 subscribers *held* for the same 60 s cost **143.8 MB**, one thirteenth as much). It **scales with the idle timeout**, which is both mechanism and mitigation: 30 s → 10 s takes the peak to **488.8 MB**. So this is an operational property with a knob rather than a defect, and **no upstream report is warranted** — the one sub-claim worth reporting, that retained session queues do not count against `cache.capacity`, needs `pool.used()` telemetry that does not exist | [test-25-isolation-under-abuse.md](test-25-isolation-under-abuse.md) |
| T26 | Cross-host fan-out: the scaling model, and whose limit the knee is | R2 — fan-out at near-zero marginal cost, the requirement the incumbent IP architectures fail | retires the caveat on every fan-out figure in the paper: all of them had the subscribers co-resident with the relay | **complete, three arms. The relay's marginal cost is small, constant and linear, and relay CPU is the first resource to bind — at the point the model predicts.** Cross-host (relay alone on a 2-vCPU c6in.large; publisher and every subscriber on a c6in.2xlarge), the same relay class carries **150 subscribers at 1,426 Mb/s** with per-subscriber delivery flat within **1.5 %**. The model: **0.806 % of a core** (r²=0.998), **1.39 MB** (r²=0.9997) and **9.84 Mb/s — one full copy** (r²=0.9996) per subscriber, giving **124–139 subscribers per core** ≈ 1.2–1.34 Gb/s of egress. **The binding resource is proven, not assumed**: EC2's four interface allowance counters stayed at **zero** throughout, the subscriber host was 39–65 % busy at every cliff, and **pinning the relay to one core moved the collapse to exactly where the slope said**, at **99.9 % of that core**. **Saturation collapses rather than degrades** — aggregate *falls* 1,184 → 528 and 964 → 46 Mb/s while relay CPU stays pinned and its RSS jumps 2.5× as queues back up — so a relay needs headroom and admission control, not a limit to run at. **Configuration outweighs everything else**: enabling UDP GSO, disabled here since a *macOS loopback* defect, cut per-subscriber CPU **29 %** and raised the ceiling by half. **T9's N = 55 knee was the test box; its slope was not** — its extrapolated 110–120 sessions per core brackets the 124–139 measured. ~6 M packets graded in-stream at **0 continuity errors** across two collapses, which also retests [#3491](https://github.com/moq-dev/moq/issues/3491)/#3515 over 450 sessions with no `Old` exit. **Limits:** two AZs in one region at 0.72 ms RTT, so this bounds *relay capacity* and says nothing about internet-scale fan-out; no pacer downstream, so delivery not conformance; 45 s per point, so not a soak; one channel | [test-26-cross-host-fanout.md](test-26-cross-host-fanout.md) |
| T27 | The per-PID liveness detector: built, made to work in a real lane, and it found a regression on its first live run | R8 (observability) — turns T24's recommendation from a graded capture into a running detector | proves the *only sufficient* detector survives the distribution path, which the architecture's observability case depends on and T24 could not test | **complete, seven arms — four synthetic, three cross-host in-lane — plus fault injection against the detector itself.** [`ts-liveness.py`](scripts/ts-liveness.py) counts access units per elementary stream in media time, streaming, constant memory, no disk, configured from the stream's own **PMT** rather than by an operator. **The headline is a coincidence of two independent instruments**: the same 60 s video suppression measures **57.212 s** live at the groomed output of a cross-host lane against T24's **57.22 s** offline on loopback — different code, different topology, three decimals apart. So the fine structure a liveness detector needs **survives a relay on another machine, the exporter's PCR regeneration and a CBR groomer**. **The audio case — invisible to every wire-observable detector in T24 — is caught and localised to the PID in 0.7–1.4 s**, the video case in **1.24 s**, and the video PID stays silent in the audio arm. **Zero alarms** on 180 s of healthy cross-host lane and 300 s of healthy broadcast material. Detection latency *is* the learned threshold, reported when the detector arms, so it is knowable per stream in advance — but it is **wider at the groomed monitoring point than in a file** (1.0–3.1 s against 1.0–1.8 s), so a monitoring point must be quoted with any latency figure. SCTE-35 and DVB subtitling are declared **unmonitorable** rather than watched, since they have no cadence to measure. **Two of the three defects found were in the detector**: one that blocks on `read()` cannot detect silence at all (it missed a *total* stall outright), and taking media time as a distance from the first PCR lets **one corrupted byte fabricate a 23,861 s outage across every PID** — seen for real at 94,847 s when a broken pipe fed it garbage. Both fixed, both with an arm that fails without the fix. **Corrects T26's client-memory reading**: held at fixed N so time is the only variable, `moq export ts` fills 50.2 → 121.9 MB over 703 s and **plateaus at 116–122 MB with drawdowns** — a cache, not the fixed ~96 MB a ramp appeared to show, and matching where T21 found it over four hours. **Its first live run on real traffic found a real fault, which is the practical case for the detector**: attached to a fixed-N soak as a false-positive control, it localised a permanent loss of video and MPEG-1 audio to those two PIDs and named the **−596.9 s delivered-clock step** that precedes them, on a lane whose *wire* stayed conformant and whose relay was transmitting 642 Mb/s. That fault is now **bisected over 53 commits to `0e61e35`, the [#3375](https://github.com/moq-dev/moq/pull/3375) merge** — the rewind fix this campaign's own T23 measurements motivated — and confirmed against its parent with two replicates per build. **The two builds carry disjoint cases**: pre-fix, a true rewind stalls everything (0.00 Mb/s); post-fix, a *continuous* timeline whose content restarts stalls video and primary audio (0.31 Mb/s). Raw captures in [`results/t27-3375/`](results/t27-3375/). **Not tested:** a frozen picture in valid advancing access units still defeats this and every other transport-layer detector | [test-27-liveness-detector.md](test-27-liveness-detector.md) |
