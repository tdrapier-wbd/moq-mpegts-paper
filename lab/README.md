...
| T23 | Which PCR timeline events the lane survives, by class | Gate 2 (conformance) across the events a permanent feed cannot avoid | grades the discontinuity as a controlled variable | complete, and re-graded against the fix it prompted | The **33-bit rollover is carried correctly end to end and always was**. Since [#3375](https://github.com/moq-dev/moq/pull/3375), which these measurements prompted, every *placed* class sits at the control's content gap. Residue: a forward jump still reaches the wire unflagged. Before the fix a rewind cost its own duration in programme with the wire showing nothing — the durable finding | [test-23-pcr-discontinuity-classes.md](test-23-pcr-discontinuity-classes.md) |
